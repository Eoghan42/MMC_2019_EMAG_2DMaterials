...
def toggle_atom_refine_position_automatically(sublattice,
    ...
    if method == 'mode':
        sublattice_scalar = scipy.stats.mode(np.round(a=sublattice_vacancy_check_list, decimals=2))[0]
    elif method == 'mean':
    ...
    if range_type == 'internal':
        ...
        raise TypeError("'internal' and 'external' are the only options for range_type")

    #checking we have some falses
    false_list_sublattice = []
    for i in range(0, len(sublattice.atom_list)):
        if sublattice.atom_list[i].refine_position == False:
            false_list_sublattice.append(sublattice.atom_list[i].refine_position)
    ...
    if filename is not None:
        sublattice.toggle_atom_refine_position_with_gui()
        plt.title('Toggle Atom Refine ' + sublattice.name + ' ' + filename + '\n Red=False', fontsize = 20)
        plt.gca().axes.get_xaxis().set_visible(False)
        plt.gca().axes.get_yaxis().set_visible(False)
        plt.tight_layout()
...
#    image_name = image.metadata.General.original_filename
    ...
    image_crop
    physical_image_crop_size_x = image_crop.axes_manager[0].scale * image_crop.axes_manager[0].size
    physical_image_crop_size_y = image_crop.axes_manager[1].scale * image_crop.axes_manager[1].size
...
#        Cropping_Variables['Image Name'] = [image_name]
        ...
        Cropping_Variables['physical_image_size X axis (%s)' %unit] = [physical_image_crop_size_x]
        Cropping_Variables['physical_image_size Y axis (%s)' %unit] = [physical_image_crop_size_y]
        ...
        Cropping_Variables_Table.to_csv('Cropping_Variables_Table.csv', sep=',', index=False)
...
def image_difference_position(sublattice_list,
    ...
    if len(atom_positions_sub_new) == 0:
        ...
        sub_new = am.Sublattice(atom_positions_sub_new, sublattice_list[0].image,
                                name=sublattice_name, color='purple')
...
def load_and_compare_images(experimental_image, simulated_image, plot_details=False):
    
    imageA = hs.load(experimental_image)
    imageB = hs.load(simulated_image)
    
    mse_number, ssm_number = measure_image_errors(imageA, imageB, plot_details=plot_details)
...
# -*- coding: utf-8 -*-
"""
Created on Sat Jan 26 15:41:22 2019
...
#filename = '003_HAADF_movie_300_4nm_MC.dm3'

##cropping the stack in the "time" axis from 60 to 20 images:
#file = hs.load(filename)
##file.plot()
##file.axes_manager.gui()
#file = file.inav[20.0:80.0]

def dm3_stack_to_tiff_stack(filename, file_extension='.dm3', save_filetype = '.tif', crop=False, crop_start=20.0, crop_end=80.0):
    ...
    new_filename = filename + save_filetype
    s.save(new_filename)
...
#dm3_stack_to_tiff_stack(filename = filename, crop=True, crop_start=50.0, crop_end=54.0)
    
    
# for after rigid registration
def save_individual_images(image_stack, output_folder='individual_images'):    
    
    # Outputting each image individually
    #images = s.stack_registered
    
    # Save each image as a 64 bit tiff
#    i=0
#    delta=1
#    while i < int(image_stack[0,0,:].shape[0]): # Find the number of images, change to an integer for the loop.
#        im = image_stack[:,:,i]
#        imwrite('64bit/images_aligned_%s.tif' %i, im)
#        i=i+delta
    ...
    i=0
    delta=1
    while i < int(image_stack_32bit[0,0,:].shape[0]): # Find the number of images, change to an integer for the loop.
...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 26 10:02:52 2019
...
#    atom_positions_1_original = am.get_atom_positions(image, calibration_separation, pca=True)
    sub1 = am.Sublattice(atom_positions_1_original, image, name=sub1_name, color=sub1_colour)
    ...
    plt.close()
    '''
    sub1.refine_atom_positions_using_center_of_mass(percent_to_nn=percent_to_nn, mask_radius=mask_radius_sub1)
    sub1.refine_atom_positions_using_2d_gaussian(percent_to_nn=percent_to_nn, mask_radius=mask_radius_sub1)
    #sub1.plot()
    
    atom_positions_1_refined = np.array(sub1.atom_positions).T
    np.save(file='atom_positions_1_refined_' + filename, arr = atom_positions_1_refined)
...
#    np.save(file='atom_positions_2_original', arr = atom_positions_2_original)
    ...
    sub2_refining.refine_atom_positions_using_center_of_mass(percent_to_nn=percent_to_nn, mask_radius=mask_radius_sub2)
    sub2_refining.refine_atom_positions_using_2d_gaussian(percent_to_nn=percent_to_nn, mask_radius=mask_radius_sub2)
    #sub2_refining.plot()
    ...
    np.save(file='atom_positions_2_refined_' + filename, arr = atom_positions_2_refined)
    
    sub2 = am.Sublattice(atom_positions_2_refined, image, name=sub2_name, color=sub2_colour)
...
#    np.save(file='atom_positions_3_original', arr = atom_positions_3_original)
    ...
    sub3_inverse = am.Sublattice(atom_positions_3_original, s_inverse, name=sub3_inverse_name, color=sub3_colour)
    ...
    sub3_inverse.refine_atom_positions_using_center_of_mass(percent_to_nn=percent_to_nn, mask_radius=mask_radius_sub2)
    sub3_inverse.refine_atom_positions_using_2d_gaussian(percent_to_nn=percent_to_nn, mask_radius=mask_radius_sub2)
    #sub3_inverse.plot()
    
    atom_positions_3_refined = np.array(sub3_inverse.atom_positions).T
    np.save(file='atom_positions_3_refined_' + filename, arr = atom_positions_3_refined)
    
    image.data = np.divide(1, image.data)
    #s.plot()
    
    sub3 = am.Sublattice(atom_positions_3_refined, image, name=sub3_name, color=sub3_colour)
    ...
    if any(false_list_sub3):
        sub3.refine_atom_positions_using_center_of_mass(percent_to_nn=percent_to_nn, mask_radius=mask_radius_sub1)
        sub3.refine_atom_positions_using_2d_gaussian(percent_to_nn=percent_to_nn, mask_radius=mask_radius_sub2)
    #sub3.plot()
...
def image_refine_via_intensity_loop(atom_lattice,
    ...
    #    file.probeStepY = file.cellDimY/atom_lattice_data.shape[0]
        ...
        calibrate_distance_and_intensity(image=simulation, 
                                         ...
                                         image_name=intensity_refine_name + "Simulation"  + loading_suffix,
                                         percent_to_nn=percent_to_nn,
                                         mask_radius=mask_radius_sub1,
                                         scalebar_true=True)
        ...
        simulation.save(intensity_refine_name + 'Filt_Simulation' + loading_suffix + '.hspy')
        ...
        counter_before_refinement = count_atoms_in_sublattice_list(
                ...
                image_name=intensity_refine_name + 'Elements' + loading_suffix)
        
        if suffix == 1:
            df_inten_refine = df_inten_refine.append(counter_before_refinement, ignore_index=True).fillna(0)
        else:
            pass
        ...
        counter_after_refinement = count_atoms_in_sublattice_list(
                sublattice_list=atom_lattice.sublattice_list,
                image_name=intensity_refine_name + 'Elements' + saving_suffix)
    
        df_inten_refine = df_inten_refine.append(counter_after_refinement, ignore_index=True).fillna(0)
        ...
        dataframe = create_dataframe_for_xyz(sublattice_list=atom_lattice.sublattice_list, element_list=element_list,
                                         x_distance=image_size_x_nm*10,
                                         y_distance=image_size_y_nm*10,
                                         z_distance=image_size_z_nm*10,
                                         save=image_name + saving_suffix,
                                         header_comment='Something Something Something Dark Side')
        
        dataframe_intensity = create_dataframe_for_xyz(sublattice_list=atom_lattice.sublattice_list, element_list=element_list,
                                         x_distance=image_size_x_nm*10,
                                         y_distance=image_size_y_nm*10,
                                         z_distance=image_size_z_nm*10,
                                         save=intensity_refine_name + image_name + saving_suffix,
                                         header_comment='Something Something Something Dark Side')
        
        # when ready:
        example_df_cif = create_dataframe_for_cif(sublattice_list=atom_lattice.sublattice_list, element_list=element_list)  
    ...
    df_inten_refine.to_csv(intensity_refine_name + 'df_inten_refine.csv', sep=',', index=False)
    ...
    plt.legend(loc=5, ncol=1, fontsize=10, fancybox=True, frameon=True, framealpha=1)
    plt.title("Refinement of Atoms via Intensity \nAll Elements", loc='left', fontsize=16, fontweight=0)
    ...
    plt.plot(df_inten_refine.index, df_inten_refine[atom_of_interest], marker='', color='orange', linewidth=4, alpha=0.7)
    ...
    for i in df_inten_refine.values[len(df_inten_refine.index)-2][1:]:
       ...
       if name != atom_of_interest:
          plt.text(text_position, i, name, horizontalalignment='left', size='small', color='grey')
    
    plt.text(text_position, df_inten_refine.Mo_1.tail(1), 'Moly', horizontalalignment='left', size='medium', color='orange')
    
    plt.title("Refinement of Atoms via Intensity", loc='left', fontsize=16, fontweight=0)
    plt.xlabel("Refinement Iteration", fontsize=16, fontweight=0)
    plt.ylabel("Count of Element", fontsize=16, fontweight=0)
    plt.tight_layout()
    ...
    atom_lattice_int_ref_name = 'Atom_Lattice_' + intensity_type + '_refined' + saving_suffix
    
    atom_lattice_int_ref = am.Atom_Lattice(image=atom_lattice_signal,
                                   name=atom_lattice_int_ref_name,
                                   sublattice_list=atom_lattice.sublattice_list)
    atom_lattice_int_ref.save(filename=intensity_refine_name + atom_lattice_int_ref_name + ".hdf5", overwrite=True)
    atom_lattice_int_ref.save(filename=atom_lattice_int_ref_name + "_intensity.hdf5", overwrite=True)
    ...
    #    print(position_refine_file, position_refine_name + '/' + position_refine_file)
        os.rename(intensity_refine_file, folder_name + '/' + intensity_refine_file)
...
def image_refine_via_position_loop(image,
    ...
    #    pr_sim.probeStepY = pr_sim.cellDimY/atom_lattice_data.shape[0]
        ...
        pr_sim.filenameOutput = filename + loading_suffix + '.mrc'
        ...
        simulation.plot()
        plt.title('Filtered_Simulation' + filename + loading_suffix, fontsize = 20)
        plt.gca().axes.get_xaxis().set_visible(False)
        plt.gca().axes.get_yaxis().set_visible(False)
        plt.tight_layout()
        ...
        if type(sub_new) == type(sublattice_list[0]):
            ...
            elements_of_sub_new = sort_sublattice_intensities(sublattice=sub_new, 
                                                              intensity_type=intensity_type, 
    ...
    #        sub_new_info = print_sublattice_elements(sub_new)
        ...
        if suffix > 4:
            if df_position_refine.diff(periods=2)[-4:].all(axis=1).all() == False:
                # if statement steps above:
                # .diff(periods=2) gets the difference between each row, and the row two above it
                # [-4:] slices this new difference df to get the final four rows
                # .all(axis=1) checks if all row elements are zero or NaN and returns False
                # .all() check if all four of these results are False
                # Basically checking if the position refinement is repeating every second iteration
                print('Finished Refinement! Repeating every second iteration.')
                break
        ...
        create_dataframe_for_xyz(
                ...
                element_list=element_list,
                 ...
                 y_distance=image_size_y_nm*10,
                 z_distance=image_size_z_nm*10,
                 filename=xyz_filename + saving_suffix,
                 header_comment='Something Something Something Dark Side')

    df_position_refine.to_csv(filename + '.csv', sep=',', index=False)

    folder_name = 'position_refined'
    createFolder('./' + folder_name + '/')
    position_refine_filenames = glob('*' + filename + '*')
    for position_refine_file in position_refine_filenames:
    #    print(position_refine_file, position_refine_name + '/' + position_refine_file)
        os.rename(position_refine_file, folder_name + '/' + position_refine_file)
    
    # Rename and sort folders for cif and xyz files
    # useful for further intensity refinement
    xyz_cif_folder = 'original_XYZ_and_CIF'
    createFolder('./' + xyz_cif_folder + '/')
    XYZ_files = glob('*' + xyz_filename + '*')
    for XYZ_file in XYZ_files[0:-1]:
        os.rename(XYZ_file, xyz_cif_folder + '/' + XYZ_file)
    
    os.rename(XYZ_files[-1], xyz_filename + '_01' + '.xyz')
    
#    return(atom_lattice_pos_ref_name)
    

#df = pd.read_csv('df_inten_refine.csv')
#go through each element and count the elements that you want
...
def compare_count_atoms_in_sublattice_list(counter_list):
    ...
    >>> compare_count_atoms_in_sublattice_list(counter_list)
...
def rigid_registration(file, masktype='hann', n=4, findMaxima='gf'):
    ...
    stack=np.rollaxis(imread(file),0,3)       # Read tiff file. Rearrange axes so final axis iterates over images
